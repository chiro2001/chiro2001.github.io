...
CDisk.prototype.articleRead = function(path, callback) {
    ...
    var context = this;
    this.cbase.read(path + 'info.json',
    function(err, data) {
        if ('null' == typeof(err)) {
            //alert('发生错误!');
            console.log(err);
        ...
        context.cbase.read(path + 'content.md',
        function(err, data) {
            if (null == typeof(err)) {
                //alert('发生错误!');
                console.log(err);
            ...
            if (key.length > 0 && key[key.length - 1] == "/") {
                ...
                function(title, content) {
                    ...
                    if (res.articles.length == range_articles) {
                        ...
                        context.cbase.write(add_domin('index.json'), json_data,
                        function(err, data) {
...
CDisk.prototype.indexRead = function(callback) {
    var context = this;
    this.cbase.read(add_domin('index.json'),
    function(err, data) {
        if (null == typeof(err)) {
            //alert('发生错误!');
            console.log(err);
